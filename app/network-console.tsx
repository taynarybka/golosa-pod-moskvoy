...
export function MetroNetworkMap({state,focusIds,compact}:{state:NetworkSession;focusIds:string[];compact:boolean}){
  ...
  const startPan=(event:React.PointerEvent<SVGSVGElement>)=>{if(event.button!==0)return;event.currentTarget.setPointerCapture(event.pointerId);dragRef.current={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,originX:pan.x,originY:pan.y,moved:false};setDragging(true);};
  const movePan=(event:React.PointerEvent<SVGSVGElement>)=>{const drag=dragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;const rect=event.currentTarget.getBoundingClientRect();const dx=(event.clientX-drag.startX)*(W/rect.width);const dy=(event.clientY-drag.startY)*(H/rect.height);if(Math.abs(event.clientX-drag.startX)>3||Math.abs(event.clientY-drag.startY)>3)drag.moved=true;setPan({x:drag.originX+dx,y:drag.originY+dy});};
  const endPan=(event:React.PointerEvent<SVGSVGElement>)=>{const drag=dragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;if(drag.moved){suppressClickRef.current=true;window.setTimeout(()=>{suppressClickRef.current=false;},0);}dragRef.current=null;setDragging(false);};
  ...
  return <div className={`${compact?"network-map compact":"network-map"}${dragging?" dragging":""}`}>
    ...
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Интерактивная карта метро, два тоннеля между станциями, позиции групп и караванов" onPointerDown={startPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
      <g transform={`translate(${tx} ${ty}) scale(${k})`}>
        ...
        {mapNodes.map(node=>{const count=playerGroups.get(node.id)||0;const npcCount=npcGroups.get(node.id)||0;const focus=focusIds.includes(node.id);const swallowed=state.world.swallowedStations.includes(node.id);return <g key={node.id} role="button" tabIndex={0} aria-label={`Станция ${node.name}`} className={`${selected===node.id?"net-station selected":"net-station"}${swallowed?" swallowed":""}`} onClick={()=>chooseStation(node.id)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")chooseStation(node.id);}}><circle className="net-station-hit" cx={node.x} cy={node.y} r={11}/><circle cx={node.x} cy={node.y} r={focus?5:2.5} fill={swallowed?"#160f1b":focus?"#d7ef53":node.color} stroke={swallowed?"#9a6ab0":"#101612"} strokeWidth={swallowed?3:1}/>{count>0&&<><circle className="net-player-badge" cx={node.x+8} cy={node.y-8} r={7}/><text className="net-player-count" x={node.x+8} y={node.y-8}>{count}</text></>}{npcCount>0&&<><circle className="net-npc-badge" cx={node.x-8} cy={node.y-8} r={6}/><text className="net-npc-count" x={node.x-8} y={node.y-8}>{npcCount}</text></>}{(focus||selected===node.id)&&<text x={node.x+8} y={node.y+4} className="net-station-name">{node.name}</text>}</g>;})}
