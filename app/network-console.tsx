...
function PlayerPhone({state,playerId,busy,act}:{state:NetworkSession;playerId:number;busy:boolean;act:(a:SessionAction)=>void}){
  ...
  return <div className="phone-screen">
    <section className="phone-identity"><RolePortrait index={player.id-1}/><div><p className="pixel-kicker">Личная карта · игрок {player.id}</p><input value={player.name} onChange={()=>{}} readOnly/><b>{role.name}</b></div><button onClick={()=>setShowRole(v=>!v)}>{showRole?"Скрыть":"Роль"}</button></section>
