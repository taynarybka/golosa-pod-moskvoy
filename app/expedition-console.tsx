...
export function ExpeditionConsole() {
  ...
  const [gameTab, setGameTab] = useState<"turn" | "bag" | "people">("turn");
  ...
  return <main className="solo-shell expedition-shell">
    ...
    <div className="solo-layout">
      ...
      <aside className="solo-command">
        ...
        <nav className="game-info-tabs" aria-label="Информация о партии"><button className={gameTab === "turn" ? "active" : ""} onClick={() => setGameTab("turn")}>Мой ход</button><button className={gameTab === "bag" ? "active" : ""} onClick={() => setGameTab("bag")}>Вещи <b>{myPlayer?.inventory.length || 0}</b></button><button className={gameTab === "people" ? "active" : ""} onClick={() => setGameTab("people")}>Участники</button></nav>
