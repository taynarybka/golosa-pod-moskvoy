...
function MapPanel({ game, setGame, addLog, onCloseSafe }: { game: GameState; setGame: React.Dispatch<React.SetStateAction<GameState>>; addLog: (s: string) => void; onCloseSafe: () => void }) {
  ...
  const selectedStory = selectedNode ? stationStories[selectedNode.id] : undefined;
    ...
    <section className="map-layout">
      <aside className="sidebar">
        ...
        <section className="side-section grow">
          ...
          {selectedNode && selectedLore && <section className="station-lore"><article className="lore-fact"><span>{selectedStory?selectedStory.title:"Городская история · редактура продолжается"}</span><p>{selectedStory?.fact||"Для этой станции вместо даты открытия будет подобрана городская легенда, необычная архитектурная деталь или история места над платформой."}</p>{selectedStory&&<a href={selectedStory.sourceUrl} target="_blank" rel="noreferrer">Источник: {selectedStory.sourceLabel} ↗</a>}</article><article className="lore-fiction"><span>После 2026 · лор игры</span><p>{selectedLore.after2026}</p></article></section>}
      ...
      <div className="map-stage">
        ...
        <svg className="metro-map" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} onWheel={(event) => { event.preventDefault(); zoomBy(Math.exp(-event.deltaY * .0012)); }} onPointerDown={startMapDrag} onPointerMove={moveMapDrag} onPointerUp={stopMapDrag} onPointerCancel={stopMapDrag} onLostPointerCapture={stopMapDrag}>
          ...
          <g transform={`translate(${transform.x} ${transform.y}) scale(${transform.k})`}>
            ...
            {tunnelEdges.flatMap((edge) => (["forward", "backward"] as Direction[]).map((direction) => { const geometry = trackGeometry(edge, direction); const key = trackKey(edge.id, direction); const mark = game.edges[key] || "normal"; return <line key={key} x1={geometry.x1} y1={geometry.y1} x2={geometry.x2} y2={geometry.y2} stroke={edge.color} className={`edge track ${mark} ${selected === key ? "selected" : ""}`} onPointerDown={(e) => e.stopPropagation()} onClick={() => selectTrack(edge, direction)} />; }))}
            {nodes.map((node) => {
              ...
              return <g key={node.id} transform={`translate(${node.x} ${node.y})`} className={`station resource-${resource.kind} ${selected === node.id ? "selected" : ""} ${isPolis ? "polis" : ""} ${branchNodeIds.has(node.id) ? "branch" : ""} ${isStart ? "start" : ""} ${isSwallowed ? "swallowed" : ""}`} onPointerDown={(e) => e.stopPropagation()} onClick={() => selectNode(node.id)}>
                ...
                {(transform.k > 2.65 || selected === node.id || isPolis) && <text className="station-name" x="9" y="-9">{isSwallowed ? `ПОГЛОЩЕНО · ${node.name}` : isPolis ? `ПОЛИС · ${node.name}` : node.name}</text>}
              </g>;
            })}
          </g>
        </svg>
...
function NpcPanel({game,setGame,addLog}:{game:GameState;setGame:React.Dispatch<React.SetStateAction<GameState>>;addLog:(s:string)=>void}) {
  ...
  return <section className="tool-page npc-page"><div className="tool-intro"><p className="eyebrow">Одноразовые спутники</p><h2>Карточки NPC</h2><p>NPC принадлежит конкретному персонажу, а не отряду. При разделении карточка остаётся в телефоне владельца. После применения услуги её текст исчезает, но NPC можно продолжать вести или принести в жертву.</p></div><div className="deck-switch">{([['all','Все'],['station','На станциях'],['hired','Наняты'],['used','Использованы'],['animal','Животные']] as const).map(([id,label])=><button key={id} className={filter===id?"active":""} onClick={()=>setFilter(id)}>{label}</button>)}</div><div className="npc-card-grid">{cards.map((npc)=>{const owner=game.npcOwners[npc.id];const used=game.npcServiceUsed[npc.id];const location=game.npcPositions[npc.id];const station=byId.get(location);const ownerPlayer=owner==null?undefined:game.players[owner];const eligible=activePlayers.map((player,index)=>({player,index})).filter(({player})=>player.lostLimbs.length<4&&(owner==null?player.position===location:player.position===ownerPlayer?.position));return <article className={`npc-card ${npc.id} ${used?"used":""} ${location==="__devoured__"?"devoured":""}`} key={npc.id}><div className="npc-art" style={npc.kind==="human"?{backgroundImage:"url('/npc-atlas-v1.png')",backgroundPosition:`${(npc.portrait%5)*25}% ${Math.floor(npc.portrait/5)*25}%`}:undefined}>{npc.kind==="animal"&&<span>{npc.id==="npc-26"?"🐕":"🐈"}</span>}<b>{npc.price}<small>◉</small></b></div><div className="npc-card-copy"><small>{npc.kind==="animal"?"Животное":"Человек"} · {npc.id}</small><h3>{npc.name}</h3><p>{npc.history}</p><div className="npc-service"><span>Одноразовая услуга</span>{used?<strong>УСЛУГА ИСПОЛЬЗОВАНА</strong>:<p>{npc.service}</p>}</div><div className="npc-status">{location==="__devoured__"?<b>Поглощён</b>:ownerPlayer?<b>Владелец: {ownerPlayer.name}</b>:<b>Станция: {station?.name||"резерв"}</b>}</div>{location!=="__devoured__"&&<div className="npc-controls"><select defaultValue="" onChange={(e)=>{if(!e.target.value)return; const next=Number(e.target.value); if(owner==null) hire(npc.id,next); else transfer(npc.id,next); e.currentTarget.value="";}}><option value="">{owner==null?"Нанять персонажем…":"Передать персонажу…"}</option>{eligible.map(({player,index})=><option value={index} key={index}>{player.name} · {byId.get(player.position)?.name}</option>)}</select>{owner!=null&&!used&&<button onClick={()=>use(npc.id)}>Применить услугу</button>}{owner!=null&&<button onClick={()=>release(npc.id)}>Оставить</button>}</div>}</div></article>;})}</div></section>;
}

function CrisisPanel({addLog}:{addLog:(s:string)=>void}) {
  const [resolved,setResolved]=useState(false);
  const crisis=crisisCards[0];
  return <section className="tool-page crisis-page">
    <div className="tool-intro"><p className="eyebrow">Один общий кризис</p><h2>Квадрат в кольце</h2><p>Это не случайная карточка, а средняя глава партии. Пока игроки не соберут фигуру на Кольцевой линии, экономика метро останавливается, а Чёрное Нечто ускоряется.</p></div>
    <div className="single-crisis-layout">
      <article className={`crisis-card crisis-main ${resolved?"resolved":""}`}>
        <div><span>Условие запуска</span><p>{crisis.trigger}</p></div>
        <div className="crisis-symbol" aria-label="Квадрат в кольце"><i/><i/><i/><i/></div>
        <h3>{crisis.title}</h3><p className="crisis-demand">{crisis.demand}</p>
        <section><span>Что должны сделать игроки</span><p className="crisis-objective">{crisis.objective}</p></section>
        <section><span>Пока кризис не решён</span>{crisis.pressure.map((effect,index)=><p key={effect}><b>{index+1}</b>{effect}</p>)}</section>
        <div className="crisis-result"><span>Результат</span><p>{crisis.result}</p></div>
        <button className="primary full" disabled={resolved} onClick={()=>{setResolved(true);addLog(`Общий кризис «${crisis.title}» разрешён: патронные дворы снова работают.`);}}>{resolved?"Кризис разрешён":"Подтвердить квадрат и завершить кризис"}</button>
      </article>
      <aside className="prophecy-board">
        <p className="eyebrow">Протокол пророчества</p><h3>Шесть символов · двенадцать толкований</h3>
        <div className="prophecy-cadence"><p><b>Старт</b>{prophecyCadence.start}</p><p><b>4 перехода</b>{prophecyCadence.first}</p><p><b>8 переходов</b>{prophecyCadence.second}</p><p><b>После кризиса</b>{prophecyCadence.crisis}</p></div>
        <div className="prophecy-grid">{prophecyConditions.map((condition)=><article key={condition.id}><strong>{condition.symbol}</strong><h4>{condition.name}</h4><details><summary>Истинное толкование</summary><p>{condition.trueText}</p></details><details><summary>Ложное толкование</summary><p>{condition.falseText}</p></details><small>Проверка ведущей: {condition.check}</small></article>)}</div>
      </aside>
    </div>
  </section>;
}
