...
export function SoloConsole(){
  ...
  const setupBeamRef=useRef<HTMLDivElement|null>(null);const setupFlashlightRef=useRef<HTMLDivElement|null>(null);
  ...
  const moveSetupFlashlight=(event:React.PointerEvent<HTMLElement>)=>{
    if(event.pointerType==="touch")return;
    setupBeamRef.current?.style.setProperty("transform",`translate3d(${event.clientX-150}px,${event.clientY-150}px,0)`);
    setupFlashlightRef.current?.style.setProperty("transform",`translate3d(${event.clientX+8}px,${event.clientY+8}px,0) rotate(-28deg)`);
    setupBeamRef.current?.classList.add("active");setupFlashlightRef.current?.classList.add("active");
  };
  const hideSetupFlashlight=()=>{setupBeamRef.current?.classList.remove("active");setupFlashlightRef.current?.classList.remove("active");};

  if(!save)return <main className="solo-setup setup-flashlight-zone" onPointerMove={moveSetupFlashlight} onPointerLeave={hideSetupFlashlight}><div ref={setupBeamRef} className="setup-flashlight-beam" aria-hidden="true"/><div ref={setupFlashlightRef} className="setup-flashlight-cursor" aria-hidden="true"><i/></div><button className="ambient-toggle setup-sound" onClick={toggleMusic}>{musicOn?"Звук: вкл":"Звук: выкл"}</button><section><p className="pixel-kicker">Москва · 2030</p><h1>Голоса ведут к Полису.<br/><span>Путь начинается здесь.</span></h1><p>Выберите персонажа и начните путь по московскому метро. У каждого путешественника — своя история и личная цель.</p><label>Выберите роль<select value={selectedRole} onChange={event=>setSelectedRole(event.target.value)}>{roleCards.map(entry=><option value={entry.id} key={entry.id}>{entry.name} · {entry.pairName}</option>)}</select></label><button className="pixel-primary" onClick={begin}>Начать экспедицию <span>→</span></button></section><div className="solo-role-preview"><RolePortrait roleId={selectedRole}/><h2>{roleCards.find(entry=>entry.id===selectedRole)?.name}</h2><p>{roleCards.find(entry=>entry.id===selectedRole)?.history}</p><b>Личная цель будет выдана после начала экспедиции.</b></div></main>;
