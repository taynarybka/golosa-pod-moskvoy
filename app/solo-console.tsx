...
export function SoloConsole(){
  ...
  useEffect(()=>{const raw=localStorage.getItem("metro-solo-v2");if(raw){try{const parsed=JSON.parse(raw) as SoloSave;setSave({...parsed,pendingTarget:parsed.pendingTarget||null,pendingEdgeId:parsed.pendingEdgeId||null,challengeAlternatives:parsed.challengeAlternatives||[],roleUses:parsed.roleUses||{}});}catch{/* Повреждённое локальное сохранение игнорируется. */}}},[]);
  useEffect(()=>{if(save)localStorage.setItem("metro-solo-v2",JSON.stringify(save));},[save]);
