...
import { getCordonProfile, itemInspectionRisk, npcCards, roleCards } from "./game-data";
...
export function SoloConsole(){
  ...
  const [infoTab,setInfoTab]=useState<"status"|"bag"|"race">("status");
  ...
  const [damageFlash,setDamageFlash]=useState(0);const lostLimbsRef=useRef<number|null>(null);
  const lostLimbs=human?.lostLimbs.length;
  useEffect(()=>{if(lostLimbs==null){lostLimbsRef.current=null;return;}if(lostLimbsRef.current!=null&&lostLimbs>lostLimbsRef.current){setDamageFlash(value=>value+1);setInfoTab("bag");}lostLimbsRef.current=lostLimbs;},[lostLimbs]);
  useEffect(()=>{setPressed(null);},[save?.phase,save?.session.round]);
  /** Короткая визуальная пауза после нажатия, чтобы кнопка успела «отозваться» до смены экрана. */
  const press=(key:string,action:()=>void)=>{if(pressed)return;setPressed(key);window.setTimeout(()=>{setPressed(null);action();},180);};
  ...
  const begin=(repeatTutorial=false)=>{const index=roleCards.findIndex(entry=>entry.id===selectedRole);setResumePending(false);setInfoTab("status");setSave(freshSave(index+1,repeatTutorial||!tutorialKnown));};
  ...
  const resolveOption=(option:ChallengeOption)=>setSave(current=>{
    ...
    if(option.outcome==="risk"){
      const succeeded=Math.random()<.52;return finishTurn({...current,session},{move:succeeded,injury:!succeeded,challenge:succeeded,bulletsSpent:option.bulletCost||0,note:succeeded?`${option.label}: риск оправдался.`:`${option.label}: попытка сорвалась.${itemNote}`});
    }
    const failureChance=itemId ? .18 : option.bulletCost ? .12 : .30;
    if(Math.random()<failureChance)return finishTurn({...current,session},{move:false,injury:true,bulletsSpent:option.bulletCost||0,note:`${option.label}: решение казалось верным, но тоннель повёл себя иначе.${itemNote}`});
  ...
  const recruitNpc=(npcId:string)=>setSave(current=>{
    ...
    const npc=npcCards.find(entry=>entry.id===npcId);if(!npc)return current;
    const session={...current.session,world:{...current.session.world,npcPositions:{...current.session.world.npcPositions},npcOwners:{...current.session.world.npcOwners},npcServiceUsed:{...current.session.world.npcServiceUsed}},players:current.session.players.map(player=>({...player,inventory:[...player.inventory],lostLimbs:[...player.lostLimbs]}))};
  ...
  if(save&&resumePending&&human&&role)return <main className="solo-resume"><section className="pixel-panel"><p className="pixel-kicker">Незаконченная экспедиция</p><div className="solo-resume-role"><RolePortrait roleId={human.roleId}/><div><h1>{role.name}</h1><p>{byId.get(human.position)?.name} · раунд {save.session.round}</p><b>{human.bullets} ◉ · {Math.max(0,4-human.lostLimbs.length)}/4 конечности</b></div></div><button className="pixel-primary" onClick={()=>setResumePending(false)}>Продолжить путь <span>→</span></button><button onClick={()=>{localStorage.removeItem(soloStorageKey);setSave(null);setResumePending(false);}}>Выбрать другую роль</button></section></main>;
  if(!save){const selected=roleCards.find(entry=>entry.id===selectedRole)||roleCards[0];return <main className="solo-setup"><button className="ambient-toggle setup-sound" onClick={toggleMusic}>{musicOn?"Звук: вкл":"Звук: выкл"}</button><section><p className="pixel-kicker">Москва · 2030</p><h1>Голоса ведут к Полису.<br/><span>Обгоните остальных.</span></h1><div className="solo-setup-objective"><strong>Главная цель</strong><p>Доберитесь до Полиса раньше большинства из одиннадцати путников.</p><div><b>1 место · суперпобеда</b><b>2–6 место · победа</b><b>7–12 место · путь завершён</b></div></div><p>Сначала выберите персонажа. Остальные правила откроются по мере путешествия.</p><div className="solo-role-picker" role="list" aria-label="Выбор персонажа">{roleCards.map(entry=><button type="button" role="listitem" aria-pressed={entry.id===selectedRole} className={entry.id===selectedRole?"selected":""} onClick={()=>setSelectedRole(entry.id)} key={entry.id}><RolePortrait roleId={entry.id}/><span>{entry.name}</span><small>{entry.pairName}</small></button>)}</div><button className="pixel-primary solo-start" onClick={()=>begin(false)}>{tutorialKnown?"Начать новую гонку":"Начать обучение"} <span>→</span></button>{tutorialKnown&&<button className="solo-repeat-tutorial" onClick={()=>begin(true)}>Повторить обучение</button>}</section><div className="solo-role-preview"><RolePortrait roleId={selectedRole}/><h2>{selected.name}</h2><p>{selected.history}</p><b>Дополнительная цель откроется после старта. Главный результат определяется местом прибытия.</b></div></main>;}
  ...
  const options=[...rawOptions.slice(optionOffset),...rawOptions.slice(0,optionOffset)];
  ...
  const availableNpcs=npcCards.filter(npc=>save.session.world.npcPositions[npc.id]===human.position&&save.session.world.npcOwners[npc.id]==null);
  const ownedNpcs=npcCards.filter(npc=>save.session.world.npcOwners[npc.id]===human.id);
  const ending=save.phase==="dead"
    ?{kind:"lost",kicker:"Поражение",title:"До Полиса вы не дошли",text:"Метро запомнило ваш путь, но живым он не завершился."}
    :save.finishRank===1
      ?{kind:"super",kicker:"Суперпобеда",title:"Вы пришли первыми",text:"Ни один из одиннадцати путников не успел добраться до Полиса раньше вас."}
  ...
  return <main className="solo-shell">{damageFlash>0&&<div key={damageFlash} className="damage-flash" aria-hidden="true"/>}
    ...
    <div className="solo-layout"><section className="solo-map"><div className="solo-race-hud"><div><small>Главная цель</small><strong>Войти в первую шестёрку</strong></div><div><small>Ваше место</small><b>№ {currentPlace} из 12</b></div><div><small>В Полисе</small><b>{arrivedCount} из 12</b></div><div><small>До Полиса</small><b>{routeDistance>=999?"путь закрыт":`${routeDistance} пер.`}</b></div></div><MetroNetworkMap state={save.session} focusIds={[human.position]} compact={false}/>{save.phase==="planning"&&<section className="solo-action pixel-panel"><p className="pixel-kicker">Ваше решение</p><button className={pressed==="stay"?"pressed":undefined} disabled={pressed!=null} onClick={()=>press("stay",()=>choose("stay",null))}><b>Остаться</b><span>{stationResources[human.position]?.label||"Локальная сцена"} · ресурс добавится в рюкзак</span></button>{availableNeighbors.map(({edge,target,status,tunnelId})=>{const targetNode=byId.get(target);const key=`${edge.id}-${target}-${tunnelId||status}`;const tunnelLabel=tunnelId==="backward"?"B":tunnelId==="forward"?"A":"";return <button key={key} className={pressed===key?"pressed":undefined} disabled={pressed!=null} onClick={()=>press(key,()=>choose("tunnel",target,edge,status,tunnelId))}><b>{tunnelLabel&&`Тоннель ${tunnelLabel} · `}Идти ↔ <i className="route-station-chip" style={{borderColor:targetNode?.color}}>{targetNode?.name}</i></b><span>{targetNode?.lineName} · {status==="caravan"?"караван ждёт: проход без испытания":edge.type==="transfer"?"переход между ветками, кордон без тоннельной карты":status==="safe"?"безопасный двусторонний тоннель, высокий шанс тихого прохода":status==="unknown"?"непонятный двусторонний тоннель, событие почти наверняка":`открытый двусторонний тоннель, 10% тихого прохода`}</span></button>;})}</section>}</section><aside className="solo-command">
      ...
      <nav className="game-info-tabs" aria-label="Информация о партии"><button className={infoTab==="status"?"active":""} onClick={()=>setInfoTab("status")}>Сейчас</button><button className={infoTab==="bag"?"active":""} onClick={()=>setInfoTab("bag")}>Рюкзак <b>{human.inventory.length}</b></button><button className={infoTab==="race"?"active":""} onClick={()=>setInfoTab("race")}>Гонка</button></nav>
      {infoTab==="status"&&<>{tutorialHint&&<section className="solo-tutorial pixel-panel"><div><span>Следующий шаг</span><b>{tutorialProgress} / {tutorialMilestones.length}</b></div><h3>{tutorialHint.title}</h3><p>{tutorialHint.text}</p></section>}<section className={`solo-goal pixel-panel ${soloGoalDone?"complete":""}`}><p className="pixel-kicker">Дополнительная цель</p><h3>{soloGoal.title}</h3><p>{soloGoal.text}</p><div><span>{soloGoalProgress} / {soloGoal.target} {soloGoal.unit}</span><b>{soloGoalDone?"Выполнено":soloGoalProgress>=soloGoal.target?"Условие собрано":"В процессе"}</b></div></section></>}
      {infoTab==="bag"&&<>{save.phase==="planning"&&human.lostLimbs.length>0&&<section className="solo-heal pixel-panel"><p className="pixel-kicker">Лечение без расхода хода</p><span>Аптечек: {human.inventory.filter(item=>item==="medkit").length}</span><div>{human.lostLimbs.map(limb=><button disabled={!human.inventory.includes("medkit")} key={limb} onClick={()=>heal(limb)}>Восстановить: {limbNames[limb]}</button>)}</div></section>}{(save.phase==="planning"||save.phase==="summary")&&(availableNpcs.length>0||ownedNpcs.length>0)&&<section className="expedition-npcs pixel-panel"><p className="pixel-kicker">NPC на станции и с вами</p>{availableNpcs.map(npc=><article key={npc.id}><b>{npc.name} · {npc.price} ◉</b><span>{npc.history}</span><small>{npc.service}</small><button disabled={human.bullets<npc.price} onClick={()=>recruitNpc(npc.id)}>{human.bullets<npc.price?`Не хватает ${npc.price-human.bullets} ◉`:"Взять с собой"}</button></article>)}{ownedNpcs.map(npc=>{const used=save.session.world.npcServiceUsed[npc.id];const healing=/возвращает одну руку|возвращает одну.*ногу|восстанавливает|конечност/i.test(npc.service);return <article key={npc.id} className={used?"used":""}><b>{npc.name} · идёт с вами</b><span>{used?"Одноразовая услуга уже использована":npc.service}</span><button disabled={used||(healing&&human.lostLimbs.length===0)} onClick={()=>applyNpcService(npc.id)}>{used?"Услуга использована":healing&&human.lostLimbs.length===0?"Некого лечить":"Применить услугу"}</button></article>;})}</section>}<section className="solo-inventory pixel-panel"><p className="pixel-kicker">Рюкзак</p><div>{inventoryGroups.map(([item,count])=><article key={item}><b>{itemCards.find(card=>card.id===item)?.title||itemNames[item]||item}</b><span>×{count}</span><small>{itemInspectionRisk[item]?`Подозрительность: ${itemInspectionRisk[item]}`:"Обычная вещь"}</small></article>)}</div></section></>}
